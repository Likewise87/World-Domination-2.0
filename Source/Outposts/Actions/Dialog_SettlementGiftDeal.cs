...
{
    /// <summary>Gift deal UI: send colony/warehouse goods (min 1000 silver) to an ally/neutral settlement.</summary>
    public class Dialog_SettlementGiftDeal : Window
    {
        ...
        public Dialog_SettlementGiftDeal(Settlement settlement)
        {
        ...
        private bool CanConfirm =>
            ...
            && SettlementGiftUtility.MeetsMinGift(GoodsMarketValue)
            && HasOrigin;
        ...
        private string ConfirmDisableReason
        {
            get
            {
                if (settlement == null || settlement.Destroyed)
                    return "TSA_WD_GiftSettlement_TargetGone".Translate();
                if (!HasOrigin)
                {
                    SettlementBuyUtility.HasPlayerPaymentOrigin(settlement.Tile, out string r);
                    return r ?? "TSA_WD_GiftSettlement_NoOrigin".Translate();
                }
                if (!SettlementGiftUtility.MeetsMinGift(GoodsMarketValue))
                    return "TSA_WD_GiftSettlement_UnderMin".Translate(SettlementGiftUtility.MinGiftSilver.ToString("F0"));
                return null;
            }
        }
        ...
        private Color MeterFillColor =>
            SettlementGiftUtility.MeetsMinGift(GoodsMarketValue) ? BarOk : BarUnder;
        ...
        private float DrawMeterBox(float x, float y, float width)
        {
            ...
            string minText = "TSA_WD_GiftSettlement_MinSilver".Translate(SettlementGiftUtility.MinGiftSilver.ToString("F0"));
            LabelAnchored(new Rect(ix, cy, iw, BoxLineH), minText, TextAnchor.MiddleLeft);
            cy += BoxLineH + MeterGap;

            float barMax = SettlementGiftUtility.GiftMeterCapSilver;
            ...
            float minMarkerT = barMax > 0f
                ? Mathf.Clamp01(SettlementGiftUtility.MinGiftSilver / barMax)
                : 0f;
            float markerX = meterBg.x + meterBg.width * minMarkerT;
            Widgets.DrawBoxSolid(new Rect(markerX - 1f, meterBg.y - 2f, 2f, meterBg.height + 4f), Color.white);
            cy += MeterH + MeterGap;

            string offeredText = "TSA_WD_GiftSettlement_Offered".Translate(
                SettlementBuyUtility.RoundSilver(GoodsMarketValue).ToString("F2"));
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            LabelAnchored(new Rect(0f, y, inRect.width, 32f),
                "TSA_WD_GiftSettlement_DialogTitle".Translate(settlement.LabelCap),
            ...
            LabelAnchored(new Rect(0f, y, inRect.width, BoxLineH),
                "TSA_WD_GiftSettlement_PayWithGoods".Translate(),
                TextAnchor.MiddleLeft);
            y += BoxLineH + 4f;
            ...
            if (Widgets.ButtonText(resetRect, "TSA_WD_BuySettlement_ResetAll".Translate()))
                ResetAllOffers();
            if (Widgets.ButtonText(fillRect, "TSA_WD_GiftSettlement_AssignToMin".Translate()))
                AssignToMin();
            TooltipHandler.TipRegion(fillRect, "TSA_WD_GiftSettlement_AssignToMinTip".Translate());
            y += 30f;
            ...
            if (Widgets.ButtonText(confirmRect, "TSA_WD_GiftSettlement_Confirm".Translate()))
            {
                var list = BuildOfferList();
                if (SettlementGiftUtility.TryLaunchGift(settlement, list, out string fail))
                {
        ...
        private void AssignToMin()
        {
            float target = SettlementGiftUtility.MinGiftSilver;
